...
const AllClientTable = () => {
  ...
  useEffect(() => {
    ...
    axiosInstance.get('/client/get-clients-paginated', { params: param })
      .then((res) => {
        setTotalCount(res.data.totalClient);
        setClientRows(res.data.clients);
        setIsLoading(false);
        console.log(res);
      })
      .catch((err) => {
  ...
  const handleDelete = (id) => {
    setIsLoading(true);
    axiosInstance.delete(`/client/delete-client/${id}`)
      .then((res) => {
  ...
  return (
    <Box width={"100%"}>
        <Box display="flex" justifyContent="right" sx={{my: 1.5}}>
            <Button startIcon={<AddCircleOutlineOutlinedIcon />} variant='contained' onClick={handleAdd}>Add Client</Button>
        </Box>
